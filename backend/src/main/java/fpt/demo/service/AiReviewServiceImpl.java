...
public class AiReviewServiceImpl implements AiReviewService {
    ...
    @Override
    public ReviewModerationResponse moderateReview(ReviewModerationRequest request) {
        String prompt = """
        You are an AI moderator for e-commerce product reviews.

        Your task is to decide whether a review should be APPROVED or REJECTED.

        Reject the review if it contains:
        - profanity, hate speech, threats, harassment
        - anti-government / extremist / violent content
        - spam, repeated nonsense text
        - suspicious links, ads, contact info, phone numbers, Telegram, WhatsApp, Facebook selling redirects
        - content unrelated to the product
        - scam or misleading promotional content

        Approve the review if it is a normal product opinion, even if it is negative.

        Important:
        - A low rating alone is NOT a reason to reject.
        - A negative opinion about product quality is still allowed.
        - Only reject if the content is unsafe, spammy, abusive, or clearly inappropriate.
        ...
        try {
            ...
            String decision = extractValue(output, "DECISION:");
            String reason = extractValue(output, "REASON:");

            return new ReviewModerationResponse(
                    decision != null ? decision : "APPROVED",
                    reason != null ? reason : "Allowed normal review"
            );
        } catch (Exception e) {
            throw new RuntimeException("Failed to moderate review: " + e.getMessage());
        }
    }
}
