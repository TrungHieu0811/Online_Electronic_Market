...
public class PayPalService {
    ...
    public String createOrder(Double amount, Integer localOrderId, String platform) {
        ...
        // Cấu hình URL quay về linh hoạt theo nền tảng
        String apiBaseUrl = "mobile".equals(platform) ? "http://10.0.2.2:8080" : "http://localhost:8080";

        Map<String, String> appContext = new HashMap<>();
        appContext.put("return_url", apiBaseUrl + "/api/users/payment/paypal-callback?orderId=" + localOrderId + "&platform=" + platform);
        appContext.put("cancel_url", apiBaseUrl + "/api/users/payment/cancel?orderId=" + localOrderId + "&platform=" + platform);
