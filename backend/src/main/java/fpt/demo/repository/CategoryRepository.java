...
public interface CategoryRepository extends JpaRepository<Category, Integer> {
  ...
  @Query("SELECT c FROM Category c WHERE c.parent IS NULL "
          + "AND (:isAdmin = true OR c.status = true) "
          + "ORDER BY c.createdAt ASC")
  List<Category> findRootCategories(@Param("isAdmin") boolean isAdmin);

  // 2. Lấy danh mục con theo slug cha
  @Query("SELECT c FROM Category c WHERE c.parent.slug = :parentSlug AND c.status = true")
  List<Category> findChildrenByParentSlug(@Param("parentSlug") String parentSlug);
