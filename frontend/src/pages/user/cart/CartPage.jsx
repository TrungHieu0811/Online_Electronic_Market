...
const CartPage = () => {
	...
	const handleUpdate = async (id, newQty) => {
		...
		if (!token) {
			...
			const updatedCart = guestCart.map((i) => (i.productId === productId ? {...i, quantity: parsedQty} : i));
			...
			<main className="max-w-7xl mx-auto px-6 lg:px-20 py-8">
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
					<div className="lg:col-span-2 space-y-6">
						{cartItems.length > 0 ? (
							cartItems.map((item) => (
								<CartItem key={item.id} item={item} onToggle={handleToggle} onUpdate={handleUpdate} onRemove={handleRemove} />
							))
						) : (
							<div className="bg-white dark:bg-slate-900 p-10 rounded-xl text-center border border-dashed border-slate-300">
								Your shopping cart is empty.{' '}
								<a href="/" className="text-primary font-bold underline hover:text-blue-700 transition-colors ml-1">
									Back to Shop!
								</a>
							</div>
						)}
