			...
			<main className="flex-1 flex flex-col min-w-0">
				...
				<div className="p-8 space-y-6 max-w-6xl mx-auto w-full">
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-sm text-slate-500 italic uppercase">
								{data.order?.orderStatus} • {data.order?.paymentStatus}
							</p>
						</div>
					</div>
					...
					{data.order?.orderStatus === 'CANCELLED' &&
						data.order?.paymentMethod === 'PAYPAL' &&
							...
							<div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
								...
								<div className="grid grid-cols-2 gap-4">
									...
									<div className="col-span-2">
										...
										<p className="font-bold text-slate-800">{data.order?.shippingAddress || 'No Address Provided'}</p>
									</div>
								</div>
							</div>
												...
												<td className="px-6 py-4">
													<div className="flex items-center gap-3">
														...
														<div className="w-12 h-12 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400">
															{item.imageUrl ? (
																<img src={item.imageUrl.startsWith('http') ? item.imageUrl : `${'http://localhost:8080/uploads'+item.imageUrl}`} alt="" className="w-full h-full object-cover rounded-lg" />
															) : (
								...
								{data.payments.length > 0 ? (
									data.payments.map((p) => (
										<div key={p.id} className="text-xs space-y-2">
											...
											<p className="text-[10px] text-slate-300 break-all bg-slate-50 p-2 rounded">ID: {p.transactionId}</p>
										</div>
									))
								) : (
								...
								{data.evidences && data.evidences.length > 0 ? (
									<div className="space-y-4">
										...
										<div className="flex gap-2">
											<span
												className={`text-[10px] px-2 py-1 rounded-full font-bold ${data.evidences[0].isValid ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}
											>
