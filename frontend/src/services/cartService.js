...
export const cartService = {
	...
	// Cập nhật số lượng
	updateQuantity: (id, quantity) => axios.put(`${API_URL}/${id}?quantity=${quantity}`, {}, getAuthHeader()),

	// Đảo trạng thái chọn (isSelected)
	toggleSelection: (id) => axios.patch(`${API_URL}/${id}/toggle-selection`, {}, getAuthHeader()),
	...
	toggleAll: (selected) => axios.patch(`${API_URL}/toggle-all?selected=${selected}`, {}, getAuthHeader()),

	// Xóa nhiêù item
	removeMultipleItems: (ids) => axios.delete(`${API_URL}/remove-multiple`, {data: ids, ...getAuthHeader()}),

	mergeCart: (guestItems) => axios.post(`${API_URL}/merge`, guestItems, getAuthHeader()),
