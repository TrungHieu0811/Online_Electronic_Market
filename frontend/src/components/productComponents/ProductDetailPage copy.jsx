...
export default function ProductDetailPage() {
	...
	const crumbs = [
		...
		{label: category?.name, path: `/category/${category?.parentSlug ?? category.name}?categoryIds=${category?.id}`},
		{label: brand?.name, path: `/category/${category?.parentSlug ?? category.name}?brandIds=${brand?.id}`},
			...
			<div className="max-w-6xl mx-auto px-4 py-6">
					...
					{crumbs
						...
						.map((c, i, arr) => (
							<React.Fragment key={i}>
								...
								>
									{i < arr.length - 1 && c.path ? <Link to={c.path}>{c.label}</Link> : c.label}
								</span>

								{i < arr.length - 1 && <FontAwesomeIcon icon={faChevronRight} style={{fontSize: 9}} />}
							</React.Fragment>
						))}
				</nav>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-10 mb-12">
					...
					<div className="flex flex-col gap-3">
						...
						<div className="relative bg-gray-50 rounded-2xl overflow-hidden aspect-square flex items-center justify-center border border-gray-100 group">
							{images[activeImg] && !hasImageError ? (
								...
								<FontAwesomeIcon icon={faBoxOpen} style={{fontSize: 64, color: '#D1D5DB'}} />
							)}

							{/* Badge góc trái — ưu tiên: Discontinued > Featured */}
							{!isActive ? (
								<div className="absolute inset-0 bg-black-40 flex items-center justify-center">
									<span className="bg-gray-500 text-white text-lg font-medium px-3 py-2 rounded-lg">Discontinued</span>
								</div>
							) : isFeatured ? (
							...
							{!isActive ? (
								...
								<div className="absolute inset-0 bg-black/40 flex items-center justify-center rounded-2xl">
									<span className="bg-white text-red-600 text-sm font-semibold px-4 py-2 rounded-lg">Out of stock</span>
								</div>
							) : null}
								...
								<div className="flex items-center gap-2">
									{images.length > 4 && (
										<button
											onClick={() => {
												...
												container?.scrollBy({left: -72, behavior: 'smooth'});
												setThumbnailScrollPos((prev) => Math.max(0, prev - 72));
											}}
											className="flex-shrink-0 p-2 hover:bg-gray-100 rounded-lg transition-colors"
											title="Scroll left"
										>
											<FontAwesomeIcon icon={faChevronLeft} style={{fontSize: 14, color: '#6B7280'}} />
										</button>
									)}
									<div
									...
									>
										{images.map((img, i) => (
											...
											>
												<img src={resolveUrl(img.imageUrl)} alt={`view ${i + 1}`} className="w-full h-full object-cover" />
											</button>
										))}
									</div>
									{images.length > 4 && (
										<button
											...
											title="Scroll right"
										>
											<FontAwesomeIcon icon={faChevronRight} style={{fontSize: 14, color: '#6B7280'}} />
										</button>
									)}
								</div>
								<p className="text-xs text-gray-500 text-center">Use arrow keys to navigate</p>
							</div>
						)}
					</div>
					...
					<div className="flex flex-col gap-4">
						{/* Brand */}
						<div className="flex items-center gap-2">
							{brand?.logoUrl && <img src={resolveUrl(brand.logoUrl)} alt={brand.name} className="h-5 object-contain" />}
							<span className="text-xs font-medium text-gray-400 uppercase tracking-wide">{brand?.name}</span>
						...
						{related && related.length > 0 && (
							<div className="mt-3 pt-3 border-t border-gray-200">
								<p className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">Product Variants</p>
										...
										>
											...
											<div className="w-15 h-15 rounded-sm overflow-hidden bg-gray-50 flex-shrink-0">
												{relatedProduct.imageList?.[0] ? (
													<img
														src={resolveUrl(relatedProduct.imageList[0].imageUrl)}
														alt={relatedProduct.variantName}
														className="w-full h-full object-cover"
													/>
												) : (
													<FontAwesomeIcon icon={faBoxOpen} style={{fontSize: 16, color: '#D1D5DB'}} />
												)}
											</div>
											...
											<div className="flex flex-col">
												...
												<span className="text-sm font-semibold text-blue-600">
													{relatedProduct.salePrice?.toLocaleString('en-US', {
														style: 'currency',
														currency: 'USD',
													})}
												</span>
						...
						<div className="bg-gray-50 rounded-2xl px-5 py-4 border border-gray-100">
							<div className="flex items-baseline gap-3 mb-2">
								<span className={`text-3xl font-bold ${hasDiscount && isActive ? 'text-orange-600' : 'text-gray-900'}`}>
									{fmtPrice(salePrice)}
								</span>
								{hasDiscount && isActive && (
									<>
										<span className="text-base text-gray-400 line-through">{fmtPrice(basePrice)}</span>
							...
							{!isActive ? (
								<div className="flex items-center gap-1.5">
									...
									<span className="text-sm font-medium text-gray-500">Discontinued — no longer available</span>
								</div>
							) : outOfStock ? (
								<div className="flex items-center gap-1.5">
								...
								<div className="flex items-center gap-1.5">
									...
									<span className="text-sm font-medium text-emerald-600">In Stock — {stockQuantity} units available</span>
								</div>
							)}
						</div>
            ...
            ${
						...
						<div className="grid grid-cols-2 gap-2 pt-1">
							<div className="flex items-center gap-2 text-xs text-gray-500 bg-gray-50 rounded-xl px-3 py-2.5">
								<FontAwesomeIcon icon={faTruck} style={{fontSize: 13, color: '#3B82F6'}} />
								Free Shipping
							</div>
							{warrantyMonths && (
								<div className="flex items-center gap-2 text-xs text-gray-500 bg-gray-50 rounded-xl px-3 py-2.5">
									<FontAwesomeIcon icon={faShield} style={{fontSize: 13, color: '#3B82F6'}} />
				...
				{description && (
					<div className="mb-10">
						<h2 className="text-lg font-semibold text-gray-800 mb-3">About this product</h2>
				...
				{attributes?.length > 0 && (
					<div className="mb-12">
						<h2 className="text-lg font-semibold text-gray-800 mb-4">Technical Specifications</h2>
							...
							<tbody>
								{attributes.map((attr, i) => (
									<tr key={i} className={`border-b border-gray-200 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
										<td className="py-3.5 px-5 text-sm font-medium text-gray-700 border border-gray-100">{attr.name}</td>
										<td className="py-3.5 px-5 text-sm text-gray-600 border border-gray-100">{attr.attrValue}</td>
									</tr>
								))}
							</tbody>
						</table>
				...
				<div className="mb-12">
					...
					<div className="border-2 border-dashed border-gray-200 rounded-2xl flex items-center justify-center py-16 text-gray-400">
						<div className="text-center">
							<FontAwesomeIcon icon={faStar} style={{fontSize: 28, color: '#E5E7EB', marginBottom: 8}} />
