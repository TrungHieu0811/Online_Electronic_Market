...
function StarRating({ rating }) {
    ...
    return (
        <div className='flex items-center gap-0.5'>
            ...
            <span className='text-xs text-gray-400 ml-1'>{Number(rating || 0).toFixed(1)}</span>
        </div>
    );
}
...
export default function ProductCard({ item, onAddToCart, onWishlist }) {
    ...
    const leftBadge = !isActive ? (
        <div className='absolute inset-0 bg-black-40 flex items-center justify-center'>
            <span className='bg-gray-50 text-black text-lg font-medium px-3 py-2 rounded-lg'>
                Discontinued
            </span>
        </div>
    ) : isFeatured ? (
<span className='absolute top-2 left-2 text-xs font-medium px-2 py-0.5 rounded bg-yellow-50 text-yellow-700'>
        ...
        <span className='absolute top-2 left-2 text-xs font-medium px-2 py-0.5 rounded bg-red-50 text-red-700'>
            Hot
        </span>
    ) : newProduct ? (
        ...
        >
            ...
            <div className='relative p-1 bg-gray-50 h-40 flex-shrink-0 flex items-center justify-center overflow-hidden'>
                ...
                {!isActive ? (
                    ...
                    <div className='absolute inset-0 bg-black/40 flex items-center justify-center'>
                        <span className='bg-red-50 text-red-700 text-xs font-medium px-3 py-1 rounded-md'>
                            Out of stock
                        </span>
                    </div>
                ) : null}
            ...
            <div className='p-3 flex flex-col flex-1'>
                ...
                <div className='flex items-center justify-between mb-1'>
                    ...
                    <span className='text-xs text-gray-400 truncate ml-1 flex-shrink-0'>
                        {category?.name}
                    </span>
                </div>
                ...
                <div className='flex items-baseline gap-1.5 mt-auto mb-3'>
                    <span
                        className={`text-base font-semibold ${hasDiscount && isActive ? 'text-orange-600' : 'text-gray-900'}`}
                    >
                        {fmtPrice(salePrice)}
                    </span>
                    {hasDiscount && isActive && (
