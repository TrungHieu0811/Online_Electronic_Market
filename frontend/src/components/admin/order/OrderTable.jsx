...
const OrderTable = ({ orders, loading }) => {
    ...
    const renderStatusBadge = (status) => {
        ...
        return (
            <span className={`px-3 py-1 rounded-full text-xs font-bold border ${colors[status] || "bg-slate-100 text-slate-600"}`}>
                • {status}
            </span>
        );
    };
                ...
                <tbody className="divide-y divide-slate-100">
                    {Array.isArray(orders) && orders.map(order => (
                        <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4 font-semibold text-slate-900">{order.user?.fullname || 'Guest'}</td>
