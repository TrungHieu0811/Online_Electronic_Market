	...
	useEffect(() => {
		const fetchStats = async () => {
			try {
				...
				setTotalOrders(typeof orders?.totalElements === 'number' ? orders.totalElements : 0);
	...
	return (
		<div className="col-span-12 grid grid-rows-2 gap-8 lg:col-span-4">
			...
			<div className="flex items-center justify-between rounded-xl bg-surface-container-lowest p-6">
				<div>
					<p className="mb-1 text-xs font-bold uppercase tracking-widest text-on-surface-variant">Total Orders</p>
					<p className="font-headline text-3xl font-extrabold text-primary">{totalOrders}</p>
				</div>
			...
			<div className="flex items-center justify-between rounded-xl bg-surface-container-lowest p-6">
				<div>
					<p className="mb-1 text-xs font-bold uppercase tracking-widest text-on-surface-variant">Member Since</p>
					<p className="font-headline text-2xl font-extrabold text-on-surface">{memberSince}</p>
				</div>
