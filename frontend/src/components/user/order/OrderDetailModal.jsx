...
const OrderDetailModal = ({ isOpen, onClose, orderId }) => {
    ...
    const handleCancel = async () => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                await axios.post(`http://localhost:8080/api/users/orders/${orderId}/cancel`, {}, {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="relative w-full max-w-3xl max-h-[92vh] overflow-y-auto rounded-3xl bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
                {/* Close Button */}
                <button onClick={onClose} className="absolute right-6 top-6 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all">
                    ...
                    <div className="p-8">
                        <div className="mb-8 border-b pb-6">
                            ...
                            <div className="mt-2 flex flex-wrap gap-x-6 gap-y-2">
                                <p className="text-slate-500 font-bold text-sm uppercase">Order ID: <span className="text-slate-900">#{order.id}</span></p>
                                <p className="text-slate-500 font-bold text-sm uppercase">Order Date: <span className="text-slate-900">
                        ...
                        <div className="mb-10 space-y-4">
                            {items.map((item) => (
                                <div key={item.id} className="flex items-center gap-6 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:shadow-md transition-all">
                                    <div className="w-20 h-20 bg-white rounded-xl p-1 border flex-shrink-0">
                        ...
                        <div className="bg-slate-900 text-white p-8 rounded-3xl space-y-4 shadow-xl">
                            ...
                            <div className="flex justify-between items-center pt-4 border-t border-slate-800">
                                ...
                                <span className="text-4xl font-black text-orange-500 tracking-tighter">${order.totalPayPrice?.toFixed(2)}</span>
                            </div>
                        </div>
