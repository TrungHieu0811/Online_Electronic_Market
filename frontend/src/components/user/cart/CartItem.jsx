...
const formatImageUrl = (url) => {
    ...
    />
			...
			<div className="flex-1 text-center sm:text-left">
				<Link to={productDetailPath} className="group transition-all duration-300 ease-in-out">
				...
				<div className="mt-4 flex items-center justify-center sm:justify-start gap-4">
					<div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
						...
						<input
							...
							onBlur={() => {
								if (item.quantity === '') onUpdate(item.id, 1);
							}} // Nếu click ra ngoài khi đang trống thì về 1
			...
			<div className="text-right">
				<p className="text-xl font-bold text-primary">${(item.product.salePrice * item.quantity).toFixed(2)}</p>
