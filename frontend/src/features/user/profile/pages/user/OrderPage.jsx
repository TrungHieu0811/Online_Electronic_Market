...
export default function OrdersPage() {
    ...
    const getItemCount = (order) =>
        order.totalQuantity || (order.orderItems || order.items || []).length;
    ...
    const needsReview = (order) =>
        order.orderStatus === 'DELIVERED' && reviewableMap[order.id] === true;
    ...
    const filteredOrders = useMemo(() => {
        ...
        if (searchTerm.trim() !== '') {
            ...
            result = result.filter((o) => {
                ...
                const productMatch = items.some((i) =>
        ...
        switch (activeTab) {
            case 'PENDING':
                return result.filter((o) => o.orderStatus === 'PENDING');
            case 'CONFIRMED':
                return result.filter((o) => o.orderStatus === 'CONFIRMED');
            case 'SHIPPING':
                return result.filter((o) => o.orderStatus === 'SHIPPING');
            case 'DELIVERED':
                return result.filter((o) => o.orderStatus === 'DELIVERED');
            case 'NEEDS_REVIEW':
                return result.filter((o) => needsReview(o));
            case 'CANCELLED':
                return result.filter((o) => o.orderStatus === 'CANCELLED');
            default:
                return result;
        }
    ...
    const totalSpent = useMemo(
        () => orders.reduce((sum, o) => sum + Number(getOrderTotal(o)), 0),
        [orders]
    );
    const activeCount = useMemo(
        () =>
            orders.filter((o) => ['PENDING', 'CONFIRMED', 'SHIPPING'].includes(o.orderStatus))
                .length,
        [orders]
    );
                ...
                <main className='ml-64 flex-1 px-8 pb-10 pt-24 lg:px-10'>
                    <div className='mx-auto w-full max-w-7xl'>
                        <header className='mb-10'>
                            <h1 className='text-4xl font-headline font-extrabold tracking-tight text-on-surface uppercase'>
                                Order History
                            </h1>
                            <p className='mt-2 font-body text-on-surface-variant'>
                                Manage and track your acquisitions.
                            </p>
                        </header>
                        ...
                        {loading ? (
                            <div className='rounded-xl bg-surface-container-lowest p-8 text-on-surface-variant shadow-sm text-center font-bold'>
                                Loading orders...
                            </div>
                            ...
                            <div className='grid grid-cols-1 gap-8 xl:grid-cols-2'>
                                {paginatedOrders.map((order) => {
                                    ...
                                    return (
                                        <div
                                            key={order.id}
                                            className={`group rounded-xl bg-surface-container-lowest p-8 shadow-sm transition-all hover:shadow-md ${showReviewButton ? 'border-2 border-transparent hover:border-amber-100/50' : ''}`}
                                        >
                                            <div className='mb-6 flex items-start justify-between gap-4'>
                                                            ...
                                                            <img
                                                                src={
                                                                    primaryImage?.startsWith('http')
                                                                        ? primaryImage
                                                                        : `http://localhost:8080/uploads${primaryImage}`
                                                                }
                                                                alt='Order'
                                                ...
                                                <span
                                                    className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${getStatusClass(order.orderStatus)}`}
                                                >
                                                ...
                                                {products.map((p, idx) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className='flex flex-col'>
                                                            <span className='line-clamp-1 font-bold text-sm'>
                                                                {p.name}{' '}
                                            ...
                                            <div className='mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                                                ...
                                                <div className='flex gap-2 flex-wrap'>
                                                    <button
                                                        ...
                                                        className='flex items-center gap-1.5 rounded-lg border border-primary/20 px-4 py-2 text-sm font-bold text-primary hover:bg-primary/5 active:scale-95 transition-all'
                                                    >
                                                    ...
                                                    {showReviewButton && (
                                                        <button
                                                            ...
                                                            onClick={() =>
                                                                navigate(
                                                                    `/profile/orders/${order.id}/review`,
                                                                    {
                                                                        state: { order }
                                                                    }
                                                                )
                                                            }
                                                            className='bg-amber-50 text-amber-700 border border-amber-200 px-4 py-2 rounded-lg text-sm font-bold hover:bg-amber-500 hover:text-white transition-all'
                                                        >
                                                            Review
                                                        </button>
                                                    )}
                        ...
                        <footer className='mt-12 flex flex-col items-center justify-center gap-6 border-t border-outline-variant/20 pt-8'>
                            <div className='flex items-center gap-3'>
                                <button
                                    ...
                                    className='flex h-10 w-10 items-center justify-center rounded-xl bg-surface-container-high disabled:opacity-30 disabled:cursor-not-allowed transition-all'
                                >
                                ...
                                <div className='flex items-center gap-2'>
                                    {getPaginationRange().map((pageIndex) => (
                                        <button
                                            ...
                                            className={`flex h-10 w-10 items-center justify-center rounded-xl font-black transition-all ${currentPage === pageIndex ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'bg-surface-container-low text-on-surface hover:bg-surface-container-high'}`}
                                        >
                                            {pageIndex + 1}
                                        </button>
                                    ))}
